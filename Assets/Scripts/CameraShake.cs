...
namespace Thinksquirrel.Utilities
{
    ...
    public class CameraShake : MonoBehaviour
    {
		...
		[SerializeField] float m_GuiShakeModifier = 01.00f;
        ...
        {	
            for (int i = 0, camerasCount = cameras.Count; i < camerasCount; i++)
            {
                ...
                if (cam.isOrthoGraphic)
                {
                    ...
                    var size = new Vector3(cam.orthographicSize / cam.aspect, cam.orthographicSize * 2f, cam.farClipPlane - cam.near);
                    Gizmos.DrawWireCube(pos, size);
                }
                else
                {
                    Gizmos.DrawFrustum(Vector3.zero, cam.fieldOfView, cam.farClipPlane, cam.near, (.7f / cam.aspect));
                }
            }
        }
